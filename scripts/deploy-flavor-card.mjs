...
const source = await readFile(resolve(process.cwd(), "contracts/FlavorCard.sol"), "utf8");
const input = {
  language: "Solidity",
  sources: { "FlavorCard.sol": { content: source } },
  settings: { outputSelection: { "*": { "*": ["abi", "evm.bytecode.object"] } } },
};
const output = JSON.parse(solc.compile(JSON.stringify(input)));
const contract = output.contracts?.["FlavorCard.sol"]?.FlavorCard;
...
console.log(`Deploying FlavorCard from ${account.address} on Base...`);
...
console.log("Set NEXT_PUBLIC_FLAVOR_CARD_CONTRACT_ADDRESS to this contract address in Vercel.");
